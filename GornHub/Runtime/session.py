...
def invoke(command, timeout=15):
    # Keep adapters and companion shutdown on the interpreter that successfully
    # started this manager, instead of re-entering Apple's Xcode-dependent shim.
    executable = sys.executable if command['executable'] == '/usr/bin/python3' else command['executable']
    return subprocess.run([executable, *command['arguments']],
