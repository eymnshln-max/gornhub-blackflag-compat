...
class OwnershipTests(unittest.TestCase):
    def test_python_adapter_uses_running_interpreter(self):
        result = s.invoke({'executable': '/usr/bin/python3',
                           'arguments': ['-c', 'import sys; print(sys.executable)']})
        self.assertEqual(result.returncode, 0)
        self.assertEqual(result.stdout.decode().strip(), s.sys.executable)

    def test_explicit_custom_executable_is_preserved(self):
        with patch.object(s.subprocess, 'run') as run:
            s.invoke({'executable': '/custom/python3', 'arguments': ['adapter.py']})
        self.assertEqual(run.call_args.args[0], ['/custom/python3', 'adapter.py'])
