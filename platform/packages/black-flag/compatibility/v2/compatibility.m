...
static void compatibilityBind(id e,SEL s,id p){NSDictionary*pass=objc_getAssociatedObject(e,&writerPassKey);id original=p;p=restoreSource([e device],p,pass);if(!objc_getAssociatedObject(original,&writerPipelineKey)[@"mesh"]){if(p==original)p=restoreTemplate([e device],p,pass);p=attachmentRepair([e device],p,pass);}p=pauseOverlay([e device],p,pass);((void(*)(id,SEL,id))previous(e,s))(e,s,p);}
...
__attribute__((constructor))static void setup(void){@autoreleasepool{
 ...
 id<MTLCommandQueue>q=[d newCommandQueue];id<MTLCommandBuffer>cb=[q commandBuffer];hook(object_getClass(cb),@selector(renderCommandEncoderWithDescriptor:),(IMP)compatibilityRender);[q release];fprintf(stderr,"Black Flag compatibility v2 loaded.\n");
}}
