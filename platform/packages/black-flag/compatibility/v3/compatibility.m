...
static id compatibilityRender(id cb,SEL s,MTLRenderPassDescriptor*d){
 ...
 for(int i=0;i<8;i++){id<MTLTexture>t=d.colorAttachments[i].texture;[formats addObject:@(t.pixelFormat)];if(t&&((t.width>=400&&t.width<=4096&&t.height>=200&&t.height<=2160)||testing))relevant=YES;}
...
__attribute__((constructor))static void setup(void){@autoreleasepool{
 ...
 id<MTLCommandQueue>q=[d newCommandQueue];id<MTLCommandBuffer>cb=[q commandBuffer];hook(object_getClass(cb),@selector(renderCommandEncoderWithDescriptor:),(IMP)compatibilityRender);[q release];fprintf(stderr,"Black Flag compatibility v3 loaded.\n");
}}
