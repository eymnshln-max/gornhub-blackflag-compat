...
__attribute__((constructor))static void setup(void){@autoreleasepool{
 ...
 id<MTLCommandQueue>q=[d newCommandQueue];id<MTLCommandBuffer>cb=[q commandBuffer];hook(object_getClass(cb),@selector(renderCommandEncoderWithDescriptor:),(IMP)compatibilityRender);[q release];fprintf(stderr,"Black Flag compatibility v6 loaded (cached hooks, 4%% presentation zoom).\n");
}}
