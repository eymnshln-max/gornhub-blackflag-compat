...
#endif
 NSDictionary*state=nil;if(matched)state=sourceDecode((uintptr_t)__builtin_frame_address(0)+16);id a=((id(*)(id,SEL))previous(d,s))(d,s);if(state){objc_setAssociatedObject(a,&sourceStateKey,state,OBJC_ASSOCIATION_RETAIN_NONATOMIC);}else if(matched){objc_setAssociatedObject(a,&sourceStateKey,nil,OBJC_ASSOCIATION_RETAIN_NONATOMIC);event(@"source-read-failed",@{@"callerOffset":@(off)});}return a;}
