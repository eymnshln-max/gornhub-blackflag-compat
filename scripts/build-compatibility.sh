...
source_dir="$repo_root/platform/packages/black-flag/compatibility/v6"
output_dir="$repo_root/build"
mkdir -p "$output_dir"
flags=(-arch x86_64 -O2 -fno-omit-frame-pointer -fblocks -framework Foundation -framework Metal)
xcrun clang "${flags[@]}" -dynamiclib -install_name @rpath/BlackFlagCompatibility.dylib "$source_dir/compatibility.m" -o "$output_dir/BlackFlagCompatibility.dylib"
codesign --force --sign - "$output_dir/BlackFlagCompatibility.dylib"
echo "Built: $output_dir/BlackFlagCompatibility.dylib"
if [[ "${1:-}" == "controls" ]]; then
  for control in hooks pause hdr mesh; do
