...
Scope: repair the readback tools, validate them against known data, then perform one bounded game capture.
...
- Corrected the exploratory `raw2png.py`, `prev.py`, `sheet.py` to share this decoder (`pixels_checked.py`). Original converters are preserved here in `converter-backups/`. Corrected contact sheet is `sheet-checked.png` beside the original sheet; historical `.png` thumbnails are not silently regenerated. Old source captures retain their timing/aliasing defect.
...
Do not return to the old `scan.m` or treat its colored output as corrupt game data. Identify the actual invoked copy path in this exact runtime before adding further hooks. The game capture ended after the single bounded run. Source-vs-target failure and its root cause remain unproven.
